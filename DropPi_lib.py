    ...
    if isinstance(relay_num, int):
        ...
        if 0 < relay_num <= NUM_RELAY_PORTS:
            ...
            if relay_num <= 4:
                ...
                bus.write_byte_data(DEVICE_ADDRESS1, DEVICE_REG_MODE1, DEVICE_REG_DATA)
            else:
                relay_num = relay_num - 4
                DEVICE_REG_DATA &= ~(0x1 << (relay_num - 1))
                bus.write_byte_data(DEVICE_ADDRESS2, DEVICE_REG_MODE1, DEVICE_REG_DATA)
        else:
...
def relay_get_port_data(relay_num):
    ...
    if 0 < relay_num <= NUM_RELAY_PORTS:
        ...
        if relay_num <= 3:
            DEVICE_REG_DATA = bus.read_byte_data(DEVICE_ADDRESS1, DEVICE_REG_MODE1)
        else:
            DEVICE_REG_DATA = bus.read_byte_data(DEVICE_ADDRESS2, DEVICE_REG_MODE1)
        # return the specified bit status
